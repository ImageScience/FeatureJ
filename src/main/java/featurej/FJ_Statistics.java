...
public class FJ_Statistics implements PlugIn, ItemListener, WindowListener {
	...
	public void run(String arg) {
		...
		slicebox = (Checkbox)checkboxes.get(size-1); slicebox.addItemListener(this);
		timebox = (Checkbox)checkboxes.get(size-2); timebox.addItemListener(this);
		channelbox = (Checkbox)checkboxes.get(size-3); channelbox.addItemListener(this);
		gd.addPanel(new Panel(),GridBagConstraints.EAST,new Insets(0,0,0,0));
		final String[] places = new String[11];
		for (int i=0; i<11; ++i) places[i] = String.valueOf(i);
		gd.addChoice("        Decimal places:",places,String.valueOf(decimals));
		...
		(new FJStatistics()).run(input,values,clear,name,channel,time,slice,decimals);
	}
...
class FJStatistics { // To avoid exceptions when ImageScience is not installed
	...
	void run(final ImagePlus input, final boolean[] values, final boolean clear, final boolean name, final boolean channel, final boolean time, final boolean slice, final int decimals) {
		...
		try {
			...
			final String title = name ? ("\t"+input.getTitle()) : "";
			...
			if (slice) {
				progressor.status("Computing statistics per slice / time / channel...");
				...
				for (cmin.c=cmax.c=0; cmin.c<dims.c; ++cmin.c, ++cmax.c) {
					for (cmin.t=cmax.t=0; cmin.t<dims.t; ++cmin.t, ++cmax.t) {
						for (cmin.z=cmax.z=0; cmin.z<dims.z; ++cmin.z, ++cmax.z) {
							statistics.run(image,cmin,cmax,mask);
							final String prelude = (++number) + title + "\t" + (cmin.c+1) + "\t" + (cmin.t+1) + "\t" + (cmin.z+1);
				...
				progressor.status("Computing statistics per time / channel...");
				progressor.steps(dims.c*dims.t);
				progressor.start();
				cmax.z = dims.z - 1;
				for (cmin.c=cmax.c=0; cmin.c<dims.c; ++cmin.c, ++cmax.c) {
					for (cmin.t=cmax.t=0; cmin.t<dims.t; ++cmin.t, ++cmax.t) {
						statistics.run(image,cmin,cmax,mask);
						final String prelude = (++number) + title + "\t" + (cmin.c+1) + "\t" + (cmin.t+1);
						textpanel.append(prelude + results());
						progressor.step();
					}
				}
				...
				progressor.status("Computing statistics per channel...");
				progressor.steps(dims.c);
				progressor.start();
				cmax.z = dims.z - 1;
				cmax.t = dims.t - 1;
				for (cmin.c=cmax.c=0; cmin.c<dims.c; ++cmin.c, ++cmax.c) {
					statistics.run(image,cmin,cmax,mask);
					final String prelude = (++number) + title + "\t" + (cmin.c+1);
					textpanel.append(prelude + results());
					progressor.step();
				}
				progressor.stop();
				...
				cmax.t = dims.t - 1;
				cmax.c = dims.c - 1;
				statistics.progressor.display(FJ_Options.progress);
				statistics.run(image,cmin,cmax,mask);
				final String prelude = (++number) + title;
				textpanel.append(prelude + results());
			}
	...
	private String headings() {
		...
		row.append("Nr");
		if (name) row.append("\tImage");
		if (slice) row.append("\tChan\tTime\tSlice");
		else if (time) row.append("\tChan\tTime");
		else if (channel) row.append("\tChan");
		if (values[4]) row.append("\tElements");
		if (values[0]) row.append("\tMin");
		if (values[1]) row.append("\tMax");
		if (values[2]) row.append("\tMean");
		if (values[6]) row.append("\tVar");
		if (values[8]) row.append("\tS-Dev");
		if (values[9]) row.append("\tA-Dev");
		if (values[3]) row.append("\tMedian");
		if (values[7]) row.append("\tMode");
		if (values[5]) row.append("\tMass");
		if (values[10]) row.append("\tL1");
		if (values[11]) row.append("\tL2");
		if (values[12]) row.append("\tSkew");
		if (values[13]) row.append("\tKurt");
	...
	private String results() {
		...
		if (values[4]) results.append("\t"+formatter.d2s(statistics.get(statistics.ELEMENTS)));
		if (values[0]) results.append("\t"+formatter.d2s(statistics.get(statistics.MINIMUM)));
		if (values[1]) results.append("\t"+formatter.d2s(statistics.get(statistics.MAXIMUM)));
		if (values[2]) results.append("\t"+formatter.d2s(statistics.get(statistics.MEAN)));
		if (values[6]) results.append("\t"+formatter.d2s(statistics.get(statistics.VARIANCE)));
		if (values[8]) results.append("\t"+formatter.d2s(statistics.get(statistics.SDEVIATION)));
		if (values[9]) results.append("\t"+formatter.d2s(statistics.get(statistics.ADEVIATION)));
		if (values[3]) results.append("\t"+formatter.d2s(statistics.get(statistics.MEDIAN)));
		if (values[7]) results.append("\t"+formatter.d2s(statistics.get(statistics.MODE)));
		if (values[5]) results.append("\t"+formatter.d2s(statistics.get(statistics.MASS)));
		if (values[10]) results.append("\t"+formatter.d2s(statistics.get(statistics.L1NORM)));
		if (values[11]) results.append("\t"+formatter.d2s(statistics.get(statistics.L2NORM)));
		if (values[12]) results.append("\t"+formatter.d2s(statistics.get(statistics.SKEWNESS)));
		if (values[13]) results.append("\t"+formatter.d2s(statistics.get(statistics.KURTOSIS)));
